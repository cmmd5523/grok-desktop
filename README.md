...
配套的 **grok-server**(独立服务)提供:

- **账号系统**:邮箱注册 → 验证码激活(验证码由管理员在管理后台查看/告知,后续可接 SMTP 自动发信)→ 登录
- **网页版用户端**:浏览器登录后聊天 + 查看自己的使用详情(对话数 / 输入输出 token / 近 30 天明细)
- **网页版管理端**:管理员登录后管理用户(增/删/改/激活/禁用/重置密码/查看验证码)+ 查看全部用户使用详情
- **桌面端接入**:桌面 app 启动需登录(同一账号),聊天后自动上报用量到服务器
